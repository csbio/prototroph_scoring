    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_CellProfiler.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * CellProfiler.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * CellProfiler.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * CellProfiler (application created by MATLAB Compiler)
        * run_CellProfiler.sh (shell script that can be run to 
    ...
    All Platforms:
        * CellProfiler.dll (shared library, file extension varies by 
          platform)
        * CellProfiler.h (library header file)
        * CellProfiler.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * CellProfiler.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * CellProfiler.xml (documentation files)
        * CellProfiler.pdb (program database file - if DEBUG option 
          selected)
        * CellProfiler.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * CellProfiler.ctf (Component Technology File archive, contains 
          compiled M-code)
        * CellProfiler_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * CellProfiler.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * CellProfiler.ctf (Component Technology File archive, contains 
          compiled M-code)
        * CellProfiler.xla (Optionally created and placed in
