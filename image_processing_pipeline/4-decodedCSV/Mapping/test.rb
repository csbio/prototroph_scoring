...
def getImageExifTime(time, fileNum)
	imageFile = '/scratch/crick/lab_share/datasets/prototroph_colony_size/090612_protrotroph_galactose_glucose/time'<<time.to_s<<'_'<<fileNum.to_s<<'.jpg'
...
def read384PlateFileNameMap(plateFileNameMap)
	...
	fileNameMap.each do |line|
		lineParts = line.chop.split("\t");
			...
			fid=File.open('outfile1.txt','w');
			fid.print(YAML::dump(lineParts));
...
begin
	...
	filePlateMap = read384PlateFileNameMap('proto scan key01.csv');
	plateDelSetMap = readDelSetNumbers('plate decoder delset map.csv');
	#print(YAML::dump(plateDelSetMap));
	#exit 1;
	orfPlateMap = readORFplateMaps('plate decoder ORF map.csv');
	badFileList = getBadFileList('prob-images.txt');
	print("File plate map");
	fid=File.open('filePlateMap.txt','w');
	fid.print(YAML::dump(filePlateMap));
	fid.close();
	print("========================================\n\n");
	print("platedelsetmap");
	fid=File.open('platedelset.txt','w');
	fid.print(YAML::dump(plateDelSetMap));
	fid.close();
	print("========================================\n\n");
	print("orf plate map");
	fid=File.open('orfPlateMap.txt','w');
	fid.print(YAML::dump(orfPlateMap));
	fid.close();
	print("========================================\n\n");
	print("bad file list");
	fid=File.open('badfilelist.txt','w');
	fid.print(YAML::dump(badFileList));
	fid.close();
	print("========================================\n\n");
